...
function RCWS(cwa_ref){const p=cwa_ref.p==='y',c=cwa_ref.c==='y';cwr=p&&c?'invest':p?'support':c?'crowd':'give';RCW();}
...
function SH(){$('sb').style.display='none';$('sht').textContent='🛍️ Shopping';var _m=document.getElementById('shop-map');if(_m)_m.style.display='block';;$('shs').textContent='Zoek op product of winkel, of gebruik de Koophulp.';si.value='';$('sc').innerHTML=`<p style="font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--mu);margin-bottom:9px;">Browse per categorie</p><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:7px;margin-bottom:18px;">${SC.map(cat=>`<div class="pl" onclick="SC2('${cat.id}')" style="padding:10px;"><div style="font-size:16px;margin-bottom:6px;">${cat.e}</div><div style="font-size:11px;font-weight:600;color:${cat.c};line-height:1.3;">${cat.l}</div><div style="font-size:9px;color:var(--mu);margin-top:2px;">${shops.filter(s=>s.c===cat.id).length} winkels</div></div>`).join('')}</div><div onclick="KH()" style="border-radius:14px;background:linear-gradient(135deg,#2d5a27,#1a7a6e);padding:14px;display:flex;align-items:center;gap:10px;cursor:pointer;"><div style="font-size:24px;">🧭</div><div><h3 style="font-size:13px;color:#fff;margin-bottom:1px;">Weet je niet waar te beginnen?</h3><p style="font-size:12px;color:rgba(255,255,255,.8);">Koophulp geeft persoonlijk advies.</p></div><div style="margin-left:auto;color:rgba(255,255,255,.6);font-size:16px;">→</div></div>`;}
...
async function LD(){if(!_DB)return;const{data:pd}=await _DB.from('projects').select('*').order('created_at',{ascending:false});if(pd)projs=pd.map(p=>({...p,desc:p.description}));const{data:sd}=await _DB.from('supporters').select('*').order('created_at',{ascending:false});if(sd)supporters=sd;if(sd&&sd.length){supporters=sd;}else{try{const ls=localStorage.getItem('fs_sup');if(ls)supporters=JSON.parse(ls);}catch(e){}}
  try{const{data:fd}=await _DB.from('fund').select('*');if(fd)fund=fd;}catch(e){}
  try{const{data:fp}=await _DB.from('fund_pot').select('*').limit(1);if(fp&&fp[0])fundpot=+fp[0].startbedrag;}catch(e){}
  RH();ROS();
  try{const ls=localStorage.getItem('fs_members');if(ls)realMembers=JSON.parse(ls);}catch(e){}
  let d=document.getElementById('h-debug');if(d&&pd)d.innerHTML='OK '+pd.length+' projecten';}
...
async function SAV(){
  ...
  if(!n||!n.value.trim()){const e=document.getElementById('pn');if(e){e.style.border='1px solid #b85450';setTimeout(()=>e.style.border='',2000);}return;}
  ...
  try{await Promise.race([_DB.from('projects').insert([{name:n.value.trim(),ind:(document.getElementById('pi')||{value:''}).value,status:(document.getElementById('pp')||{value:'Idee'}).value,description:(document.getElementById('pd')||{value:''}).value.trim(),needs}]),new Promise((_,r)=>setTimeout(()=>r('timeout'),8000))]);}catch(e){console.error('Project:',e);return;}
  document.getElementById('hf').style.display='none';await LD();
}
...
async function ADS(){
  ...
  try{localStorage.setItem('fs_sup',JSON.stringify(supporters));}catch(e){}
  n.value='';OSC.forEach(([v])=>{const inp=document.getElementById('osc-'+v);if(inp)inp.value='';});
...
async function loadRealMembers(){try{const l=JSON.parse(localStorage.getItem('fs_members')||'[]');if(l.length){realMembers=l;return;}}catch(e){}try{const r=await Promise.race([fetch(SURL+'/rest/v1/members?select=*&order=created_at.asc',{headers:{'apikey':SKEY,'Authorization':'Bearer '+SKEY}}),new Promise((_,rej)=>setTimeout(()=>rej('t'),5000))]);if(r&&r.ok){const d=await r.json();if(d&&d.length)realMembers=d;}}catch(e){}}
